...
export POCHOIR_STORE="${1:-edge_bc_validation/store_edge_bc_true_fixed_0p05mm}"
...
############################################################################
## PART A: DRIFT FIELD  (full-depth single 0.05mm solve, 2cm drift / 3cm volume)
############################################################################
export POCHOIR_LOG="${POCHOIR_STORE}/pochoir_driftfield.log"

gen="pcb_drift_pixel_with_grid"
dcfg="example_gen_pcb_drift_pixel_larpix_v2a_wogrid.json.0p05mm"


## Single full-depth drift domain: 88x88 periodic tile = EXACTLY the 4.4mm pitch
## @0.05mm.  At this spacing the true LArPix lengths need no snapping:
## pixelSize 3.5 (70 cells) + pixelGap 0.9 (18 cells) = 88 cells = 4.4mm.
## 601 planes -> 600 cells x 0.05mm = 30.0mm; the cathode is on the last plane
## (index 600, z=30mm) and the path start sits at driftZDepth=29.9mm (index
## 598), just below it.  Pixel plane low edge at 9.9mm -> 2cm drift length.
want domain/drift \
     pochoir domain --domain domain/drift \
     --shape=88,88,601 --spacing '0.05*mm'
...
############################################################################
## PART C: WEIGHTING FIELD  (full-depth single 0.05mm solve, 5x5 pixel grid)
############################################################################
export POCHOIR_LOG="${POCHOIR_STORE}/pochoir_weightingfield.log"

wgen="pcb_pixel_with_grid"
wcfg="example_gen_pixel_larpix_v2a_wogrid.json.0p05mm"


## Weighting domain: exactly 5x the drift domain transversely --
## 5 x 88 = 440 planes @0.05mm, i.e. 5 pixels x 4.4mm pitch = 22.0mm, the full
## 5x5 pad array with no padding.  Same z extent as the drift domain (601).
want domain/weight \
     pochoir domain --domain domain/weight \
     --shape=440,440,601 --spacing '0.05*mm'
