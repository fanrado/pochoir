...
def main():
    ...
    def efield_batch(P):
        ...
        Plain central difference of the interpolated potential (sample points
        clamped inside the bbox; one-sided when a neighbour falls outside).  NO
        insulator/mask correction: matches the pure E=grad(phi) production drift
        whose only insulator condition is the solver's Neumann FR4 BC
        (pochoir-w3x9).
        """
        M = P.shape[0]
        E = np.zeros((M, ndim))
