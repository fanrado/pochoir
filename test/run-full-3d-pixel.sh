...
## ---------------------------------------------------------------------------
## Step 4: near-field fine weighting solve (0.05mm), seeded + pinned interface (sweep 0)
## ---------------------------------------------------------------------------
...
## Step 4b: overlapping-Schwarz refinement for a continuous interface
## ---------------------------------------------------------------------------
# Mirror PART A: starting from the sweep-0 near solve (Steps 2-4, whose
# intermediate store keys are preserved for resume), alternately re-solve the
# full coarse far domain (interior plane one coarse cell below z=20mm pinned to
# the near solution) and the fine near domain (interface pinned to the far
# solution), iterating to convergence.  The result is continuous in value AND
# gradient across z=20mm.  edges/precision match the weighting fdm above
# (fix,fix,fix; near 2e-10, far 2e-7).  The weighting potential ranges 0..1, so
# the Schwarz tolerance is 0.001*V (0.1% of the unit range), not drift's 1*V.
# Updates potential/weight_near and writes the consistent far field.
...
##         onto the Schwarz-updated far field (potential/weight_far).
