#!/bin/bash
#
# Task10a: 2cm drift + weighting + induced-current run, NO shield grid,
# 3.8mm pixel / 0.6mm gap / 0.7mm chamfer, full-depth single-pass, uniform 0.1mm.
# ---------------------------------------------------------------------------
# Same wiring as run-task10-drift-2cm-01mm.sh (GridHoleShape "None" -> no shield
# grid), with the pad chamfer radius raised 0.4mm -> 0.7mm (chamferMode
# "dynamic": chamfer is specified in mm, converted to cells by the generator).
#
...
#
# Geometry at 0.1mm: pixel pitch 4.4mm (3.8mm pad + 0.6mm gap), pp_loweredge=99
...
#
# Usage: ./run-task10a-drift-2cm-chamf07.sh [STORE_DIR]

set -e

export POCHOIR_STORE="${1:-store_task10a_drift_2cm_chamf07}"
...
dcfg="example_gen_pcb_drift_pixel_task10a_insul.json"

echo "=== drift field: no shield grid, 3.8mm pixel / 0.6mm gap / 0.7mm chamfer, no-flux FR4 insulator (NO epsilon) ==="
...
wcfg="example_gen_pixel_with_grid_task10a_insul.json"
...
date
echo "=== DONE: Task10a 2cm 0.1mm drift + weighting + induced-current run complete (3.8mm/0.6mm/0.7mm chamfer, no shield grid) ==="
