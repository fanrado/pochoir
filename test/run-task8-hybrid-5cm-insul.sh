#!/bin/bash
# ===========================================================================
# TASK 8 : 5 cm drift, HYBRID solver (coarse 0.4 mm + fine 0.05 mm),
#          DYNAMIC (physical-unit) 0.7 mm chamfer, INSULATING-SURFACE (no-flux
#          Neumann) FR4 boundary ACTIVE.  NO epsilon.  Full pipeline:
#          PART A drift FIELD + drift PATHS + overshoot check;
#          PART B WEIGHTING FIELD (unit probe, multi-pixel, no-flux insulator);
#          PART C INDUCED CURRENT (Ramo: weighting field x drift paths).
#
# Purpose: verify the insulating-surface overshoot fix (EPIC pochoir-ktj0,
#          validated at 2 cm in pochoir-rqgd) at a longer, more realistic 5 cm
#          drift.  A 5 cm gap at 0.05 mm is too large for a single fine domain,
#          so this SEPARATES the near and far field (hybrid): a coarse 0.4 mm
#          full-depth solve seeds a fine 0.05 mm near-field solve around the
#          pad, which is stitched back onto the coarse far field.
#
# Geometry (dynamic 0.7 mm chamfer, GridHoleShape "None" -> plain Laplace on E
...
# `near-far-solve` step used by Task2/Task5 is intentionally OMITTED here: it
# does not yet carry the insulator mask (it hardcodes epsilon=None and re-solves
# potential/near, which would DISCARD the near insulator field) -- see beads
# issue for adding insulator support to near-far-solve.  Skipping it is safe for
# THIS validation because the overshoot is a pad-local (z=10 mm) phenomenon far
# below the z=20 mm near/far interface, which is already pinned Dirichlet from
# the coarse bulk via `near-bc`.
#
# Output folder: store_task8_hybrid_5cm_insul/
# Run from test/:  ./run-task8-hybrid-5cm-insul.sh
# ===========================================================================
set -e
cd "$(dirname "$0")"
ROOT="$(cd .. && pwd)"
...
export POCHOIR_STORE=store_task8_hybrid_5cm_insul
export POCHOIR_LOG="${POCHOIR_STORE}/pochoir.log"
CFG=example_gen_pcb_drift_pixel_hybrid5cm_insul.json
...
$PY compute_chamfer_removed.py --store "$POCHOIR_STORE" --mode dynamic --spacing 0.05 --chamfer-mm 0.7 \
    --pad-z 10 --drift 50 --total 60 --cathode -2500 --hybrid --coarse-spacing 0.4 \
    --title "Task 8 -- 5 cm drift, hybrid (0.4+0.05 mm), dynamic 0.7 mm chamfer, insulating-surface FR4 (NO epsilon)"
...
## 10x10 launch grid per pixel, near the cathode plane (z=59.5mm).
...
## PART B: WEIGHTING FIELD  (unit probe, multi-pixel, no-flux insulator)
##
## The weighting potential is a UNIT probe: target pixel pad = 1 V, ALL other
## conductors = 0 V, solved on a MULTI-PIXEL, NON-periodic (fix,fix,fix) domain
## (5 pixels = 22 mm transverse) so phi_w decays to ~0 at the edges -- unlike
## the drift field's single-pixel periodic 4.4 mm tile.  The SAME no-flux FR4
## insulator mask as the drift field is applied (W1 pochoir-epbh + near-far-solve
## plumbing pochoir-oz2l); NO epsilon.  Pad plane aligned to the drift geometry
## (z = 10 mm) via example_gen_pixel_with_grid_task8_insul.json.
############################################################################
...
WCFG=example_gen_pixel_with_grid_task8_insul.json
date
...
## PART C: INDUCED CURRENT  (Ramo: weighting field x drift paths)
##
## Uses the insulator-BC drift paths from PART A (paths/drift3d) and the
## insulator-consistent weighting field (potential/weight3d).  induce-pixel
## SHIFTS the single-tile drift paths across the pixel grid (--npixels) and
## integrates i(t) = q E_w(x(t)) . v(x(t)) per pixel.
############################################################################
...
echo "=== TASK 8 DONE (drift + overshoot + weighting + induced current): $POCHOIR_STORE ==="
