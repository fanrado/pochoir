...
#          drift gap above the pad and the region below the FR4.  The FDM
#          solve is now Poisson (harmonic-mean permittivity) instead of pure
#          Laplace: NEPS=1 passes the generator epsilon to the solver.
#
...
echo "### [2/3] drift field (Poisson w/ FR4 epsilon) + drift paths (near-field-only, 0.05 mm, z=0..30 mm)"
NEPS=1 NSHAPE=88,88,601 NSPACING='0.05*mm' NLAUNCH=29.8 NTWIN=30 \
